...
const nav = ({isActive}:{isActive:boolean}) => `app-nav-link ${isActive ? 'app-nav-link-active' : ''}`
...
function Layout() {
  ...
  return <main className="app-shell"><aside className="app-sidebar"><div className="app-brand"><span className="app-brand-mark">Z</span><h1>Z-Image</h1></div><nav className="mt-8 grid gap-1"><NavLink className={nav} to="/dashboard">Workspace</NavLink><NavLink className={nav} to="/datasets">Datasets</NavLink><NavLink className={nav} to="/jobs">Runs</NavLink><NavLink className={nav} to="/settings">Settings</NavLink></nav><div className="app-sidebar-footer"><span className="app-status-dot"/>API online</div></aside><div className="app-body"><section className="app-content"><Outlet /></section></div></main>
}
...
function Dashboard() {
  ...
  return <><header className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-2xl font-semibold tracking-tight">Workspace</h2><Link className="whitespace-nowrap rounded-lg bg-slate-900 px-3 py-2 text-xs font-semibold text-white transition hover:bg-slate-700" to="/datasets">New dataset</Link></header>{!activeJob?<section className="empty-workspace mt-5"><div><h3 className="text-sm font-semibold text-slate-900">No active training</h3><p className="mt-2 max-w-xl text-sm leading-relaxed text-slate-500">Open a dataset to upload images, review captions, and start a training run.</p><Link className="mt-5 inline-flex whitespace-nowrap rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-50" to="/datasets">Open datasets</Link></div></section>:<><section className="panel mt-5"><div className="flex items-center justify-between"><div><h3 className="text-sm">Current training run</h3><p className="mt-0.5 text-[11px] text-slate-500">Job {activeJob}</p>{displayStage&&monitorStatus==='running'&&<p className="mt-1 text-[11px] font-medium text-cyan-700">{displayStage}</p>}</div><span className="muted uppercase">{s.training.status==='sampling'?'Generating preview…':monitorStatus}</span></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-gradient-to-r from-cyan-500 to-violet-500" style={{width:`${progress}%`}}/></div><div className="mt-2 flex justify-between text-xs text-slate-600"><span>{currentStep.toLocaleString()} / {totalSteps.toLocaleString()} steps</span><span>{currentLoss !== undefined ? `loss ${currentLoss.toFixed(5)}`:'waiting for first step'}</span></div><div className="mt-3 border-t border-slate-100 pt-3"><LossChart points={points}/></div></section><section className="panel mt-4"><div className="flex items-center justify-between"><div><h3 className="text-sm">Validation images</h3><p className="mt-0.5 text-[11px] text-slate-500">Baseline and checkpoint outputs use the same prompt and seed.</p></div><span className="text-[11px] text-slate-500">{samples.length} images</span></div>{samples.length?<div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">{samples.map((path:string)=><a key={path} href={`${API}/files/${path}`} target="_blank" className="overflow-hidden rounded-xl border border-slate-200 bg-slate-100 hover:border-cyan-400"><img className="aspect-square w-full object-cover" src={`${API}/files/${path}`}/><span className="block truncate bg-white px-2 py-1 text-[10px] text-slate-600">{path.split('/').at(-1)}</span></a>)}</div>:<p className="mt-3 text-xs text-slate-400">Validation images will appear here when the selected plan enables them.</p>}</section></>}</>
}
